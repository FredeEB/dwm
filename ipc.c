 ...
 * handle.
 */
static void ipc_reply_prepare_send_message(yajl_gen gen, IPCClient *c, IPCMessageType msg_type) {
    const unsigned char *buffer;
    size_t len = 0;
 ...
 */
static int ipc_validate_run_command(IPCParsedCommand *parsed, const IPCCommand actual) {
 ...
 * Returns -1 otherwise
 */
static int ipc_parse_subscribe(const char *msg, IPCSubscriptionAction *subscribe, IPCEvent *event) {
...
static int ipc_run_command(IPCClient *ipc_client, char *msg) {
    ...
    if (res < 0) {
        if (res == -1)
            ipc_prepare_reply_failure(ipc_client, IPC_TYPE_RUN_COMMAND, "%u arguments provided, %u expected", parsed_command.argc,
                                      ipc_command.argc);
        else if (res == -2)
 ...
 */
static int ipc_get_dwm_client(IPCClient *ipc_client, const char *msg, const Monitor *mons) {
    Window win;
...
IPCClient *ipc_get_client(int fd) { return ipc_list_get_client(ipc_clients, fd); }
...
int ipc_read_client(IPCClient *c, IPCMessageType *msg_type, uint32_t *msg_size, char **msg) {
    ...
    if (ret < 0) {
        ...
        if (ret == -1 && (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)) return -2;
...
void ipc_client_focus_change_event(int mon_num, Client *old_client, Client *new_client) {
    yajl_gen gen;
    ipc_event_init_message(&gen);
...
void ipc_focused_title_change_event(const int mon_num, const Window client_id, const char *old_name, const char *new_name) {
    yajl_gen gen;
    ipc_event_init_message(&gen);
...
void ipc_focused_state_change_event(const int mon_num, const Window client_id, const ClientState *old_state, const ClientState *new_state) {
    yajl_gen gen;
    ipc_event_init_message(&gen);
    dump_focused_state_change_event(gen, mon_num, client_id, old_state, new_state);
